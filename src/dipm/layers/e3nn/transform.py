...
from dipm.layers.e3nn.utils import order_mask, rescale_matrix
...
class SO3Rotation:
    ...
    def __init__(self, lmax: int, mmax: int, *, dtype: Dtype = jnp.float32):
        self.lmax = lmax
        ...
        self.wigner, self.wigner_inv = None, None
    ...
    def set_wigner(
        ...
        '''Compute the Wigner-D matrices from the Euler angles then cache them.'''
        size = (self.lmax + 1) ** 2
        ...
        self.wigner = jax.lax.stop_gradient(wigner)
        self.wigner_inv = jax.lax.stop_gradient(jnp.transpose(wigner, (0, 2, 1)))
    ...
    def rotate_inv(self, embedding):
        ...
        wigner_inv = self.wigner_inv * self.rotate_inv_rescale
        return jnp.matmul(wigner_inv, embedding)
...
class SO3Grid:
    ...
    def __init__(
        ...
        *,
        ...
        self.to_grid_mat = jax.lax.stop_gradient((to_grid_mat * rescale_mat)[:, :, self.mask])
        self.from_grid_mat = jax.lax.stop_gradient((from_grid_mat * rescale_mat)[:, :, self.mask])
