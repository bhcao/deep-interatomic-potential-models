...
def _in_str(sub, string: str | int):
...
def _weight_decay_mask(params: nnx.State) -> nnx.State:
    params = flatten_mapping(params)

    linear_down_exists = any(any(_in_str("linear_down", ki) for ki in k) for k in params)
    symmetric_contraction_exists = any(
        any(_in_str("SymmetricContraction", ki) for ki in k) for k in params
    )

    # Only apply the mask if one of them exists
    if linear_down_exists or symmetric_contraction_exists:
        mask = {
            k: any(_in_str("linear_down", ki) or _in_str("SymmetricContraction", ki) for ki in k)
            for k in params
        }
        return nnx.State(unflatten_mapping(mask))

    # Keep same with input params (nnx.State) to avoid jax.tree.map error.
    return nnx.State(unflatten_mapping(dict.fromkeys(params, True)))
...
def get_default_mlip_optimizer(
    ...
    :meth:`~dipm.training.optimizer.get_mlip_optimizer_chain_with_flexible_base_optimizer`
