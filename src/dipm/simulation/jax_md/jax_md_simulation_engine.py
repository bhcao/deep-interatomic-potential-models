...
from dipm.simulation.jax_md.states import EpisodeLog, JaxMDSimulationState, SystemState
from dipm.simulation.simulation_engine import ForceFieldPredictor, SimulationEngine
from dipm.simulation.temperature_scheduling import get_temperature_schedule
from dipm.simulation.utils import create_graph_from_atoms
from dipm.typing.graph_definition import GraphEdges
...
class JaxMDSimulationEngine(SimulationEngine):
    ...
    def _initialize(
        ...
        graph = self._init_base_graph(
            atoms, senders, receivers, force_field.allowed_atomic_numbers, self.task_index
        )
    ...
    def _reallocate_neighbors(self) -> None:
        ...
        # Update _pure_simulation_step_fun with new n_edge
        ...
        graph = self._init_base_graph(
            self.atoms, senders, receivers, self.force_field.allowed_atomic_numbers, self.task_index
        )
        model_calculate_fun = self._get_model_calculate_fun(
    ...
    def _get_model_calculate_fun(
        graph: jraph.GraphsTuple, force_field_model: ForceFieldPredictor, is_batched_sim: bool
