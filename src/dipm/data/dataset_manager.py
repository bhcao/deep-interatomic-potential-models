...
def _process_datasets_in_parallel(
    ...
    with mp.Pool(processes=min(len(datasets), mp.cpu_count())) as pool:
        results = [
...
class DatasetManager:
    ...
    def prepare_datasets(self, dataset_info: DatasetInfo | None = None) -> DatasetInfo:
        ...
        train_datasets = self.ds_create.get_train_datasets(train_filter_fn)
        ...
        valid_test_datasets = (
            self.ds_create.get_valid_datasets(valid_test_filter_fn) +
            self.ds_create.get_test_datasets(valid_test_filter_fn)
        )

        _process_datasets_in_parallel(
            valid_test_datasets, desc="Analyzing validation/test set"
        )
