    ...
    def _worker(self, dataset: Dataset, queue_in: mp.Queue, queue_out: mp.Queue):
        ...
        while True:
            ...
            if isinstance(result, list):
                outs = []
                for i in result:
                    outs.append(self.post_process_fn(i))
                queue_out.put(outs)
            else:
                queue_out.put(self.post_process_fn(result))
    ...
    def _load(self, indices, ds_indices):
        ...
        if self.parallel:
            if self._loader is None:
                self._loader = _ParallelLoader(self.datasets, self.post_process_fn)
                self._loader.start()
        ...
        if isinstance(indices, int):
            ...
            if self.post_process_fn is not None:
                item = self.post_process_fn(item)
            return item
        ...
        for idx, ds_idx in zip(indices, ds_indices):
            ...
            if self.post_process_fn is not None:
                data = [self.post_process_fn(i) for i in data]
            results.append(data)
        return results
...
class Subset(Dataset):
    """Subset of a dataset with a given slice. If input is a ConcatDataset, it will be flattened."""
    ...
    def __new__(cls, dataset: Dataset, start: int, length: int):
        ...
        if dataset.shuffle:
            concat_ds = ConcatDataset(
                ...
                post_process_fn=dataset.post_process_fn
            )
