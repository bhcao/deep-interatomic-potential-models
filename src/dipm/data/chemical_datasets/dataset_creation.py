...
class DatasetCreation:
    ...
    def get_train_datasets(self, post_process_fn: _PostProcessFn = None) -> list[Hdf5Dataset]:
        """List of training datasets."""
        return self._get_datasets(self.config.train_dataset_paths, post_process_fn)

    def get_valid_datasets(self, post_process_fn: _PostProcessFn = None) -> list[Hdf5Dataset]:
        """List of validation datasets."""
        if self.config.valid_dataset_paths is None:
            return []
        return self._get_datasets(self.config.valid_dataset_paths, post_process_fn)

    def get_test_datasets(self, post_process_fn: _PostProcessFn = None) -> list[Hdf5Dataset]:
        """List of test datasets."""
        if self.config.test_dataset_paths is None:
            return []
        return self._get_datasets(self.config.test_dataset_paths, post_process_fn)
