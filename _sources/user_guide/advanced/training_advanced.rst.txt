...
* Creating an instance of
  :py:class:`TrainingLoopConfig <dipm.training.configs.TrainingLoopConfig>`
  (can be accessed via `TrainingLoop.Config`, too).
* Optionally (has a default): Creating an instance of
  :py:class:`TrainingIOHandler <dipm.training.training_io_handler.TrainingIOHandler>`
  (see :ref:`below <training_io_handler>` for details).

Once these objects are set up, we can create an instance of
:py:class:`TrainingLoop <dipm.training.training_loop.TrainingLoop>` and start
the training run:
...
The final ForceFieldPredictor instance can be accessed after the run like this:

.. code-block:: python

    # Obtain the model at the last moment without EMA
    final_force_field = training_loop.force_field

    # Obtain the model after EMA at the last moment
    ema_force_field = training_loop.ema_model

    # Obtain the best model during the training process
    best_force_field = training_loop.best_model

If you want to save a trained force field not only via the checkpointing API described
further below, you can also use the function
:py:func:`save_model() <dipm.utils.model_io.save_model>` to save it
as a lightweight safetensors archive in case you only want to use it for inference tasks later,
...
using the
:py:func:`test() <dipm.training.training_loop.TrainingLoop.test>` method of the
:py:class:`TrainingLoop <dipm.training.training_loop.TrainingLoop>` instance.
...
:py:class:`Loss <dipm.loss>`. We currently implement two losses, the
Mean-Squared-Error loss (:py:class:`MSELoss <dipm.loss.loss.MSELoss>`), and the
Huber loss (:py:class:`HuberLoss <dipm.loss.loss.HuberLoss>`), which are both losses
...
number (base class: :py:class:`WeightedEFSLoss <dipm.loss.loss.WeightedEFSLoss>`).
...
the :py:class:`call method <dipm.loss.loss.WeightedEFSLoss.__call__>` of the class
:py:class:`WeightedEFSLoss <dipm.loss.loss.WeightedEFSLoss>` for more information on
...
:py:class:`TrainingLoop <dipm.training.training_loop.TrainingLoop>`
...
:py:class:`OptimizerConfig <dipm.training.optimizer_config.OptimizerConfig>` object that
...
:py:func:`get_default_mlip_optimizer <dipm.training.optimizer.get_default_mlip_optimizer>`
and
:py:class:`OptimizerConfig <dipm.training.optimizer_config.OptimizerConfig>`
...
:py:class:`TrainingIOHandlerConfig <dipm.training.configs.TrainingIOHandlerConfig>`,
...
(see :py:func:`load_parameters_from_checkpoint() <dipm.models.params_loading.load_parameters_from_checkpoint>`).
...
.. code-block:: python
    ...
    import os
    from dipm.training import TrainingIOHandler
...
the saved :py:class:`DatasetInfo <dipm.data.dataset_info.DatasetInfo>`
...
.. code-block:: python
    ...
    dipm.training.training_io_handler import LogCategory
...
:py:class:`LogCategory <dipm.training.training_io_handler.LogCategory>`
...
to the IO handler,
:py:func:`log_metrics_to_table() <dipm.training.training_loggers.log_metrics_to_table>`,
...
:py:func:`log_metrics_to_line() <dipm.training.training_loggers.log_metrics_to_line>`,
...
:py:func:`convert_mse_to_rmse_in_logs() <dipm.training.training_loggers.convert_mse_to_rmse_in_logs>`,
which is a helper function we provide for this task.

Note that it is possible to omit the `io_handler` argument in the
:py:class:`TrainingLoop <dipm.training.training_loop.TrainingLoop>` class. In that case,
